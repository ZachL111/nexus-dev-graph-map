...
`nexus-dev-graph-map` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies graph behavior through round-trip fixtures, with lossless normalization checks and no network dependency.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
